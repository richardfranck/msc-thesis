...
class ExtractionEvaluator:
    ...
    def _determine_whether_estimable(self, nr_obs, nr_interior_knots):
        """Report whether the spline basis is identified at this grid cell.

        A given cell is not identified if it implies B = K + 4 coefficients, but 
        has less than B observations. The build design has equally spaced 
        observations so there is an observation in the domain of each basis. 
        If the first condition is satisfied, the second is therefore satisfied
        by construction. 
        ...
        Returns:
            bool: True where the design matrix has full column rank.
        """
        # A cubic spline with K interior knots carries B = K + 4 coefficients
        return nr_interior_knots + 4 <= nr_obs
    ...
    def _fit_ols_splines(self, nr_obs, nr_interior_knots):
        ...
        Since there is no model for mapping covariates X to spline coefficents
    ...
    def evaluate_extraction(self, nr_obs, nr_interior_knots, zeta_rel=0.0,
                            upsilon_rel_1=0.0, upsilon_rel_2=0.0, upsilon_rel_prune=0.0,
                            do_prune=False):
        ...
        Returns:
            ...
            Where K + 4 exceeds nr_obs, the basis is not identified by the
            observations and every value is returned as None. 
        """
        # Step 0: Return none if the provided configuration cannot be estimated
        if not self._determine_whether_estimable(nr_obs, nr_interior_knots):
            return {"accuracy": None, "distance": None,
        ...
        for w in fit["coefficients"]:
            summary = extract_shape_summary(
                ...
                zeta_rel=zeta_rel,
                upsilon_rel_1=upsilon_rel_1,
                upsilon_rel_2=upsilon_rel_2,
        ...
        truth = get_analytic_shape_summary(self.dataset, upsilon_rel_1, upsilon_rel_2,
    ...
    def sweep_observations(self, variable_nr_obs, fixed_nr_knots, zeta_rel, fixed_upsilon_rel_1, fixed_upsilon_rel_2, upsilon_rel_prune, do_prune):
        ...
        nr_obs. The other inputs (nr_knots, upsilon_rel_1, upsilon_rel_2) remain fixed.
        
        Note: We compare the analytic shape summary and the shape summary our extractor
        ...
        Args:
            ...
            fixed_upsilon_rel_1 (float): Relative slope threshold (fixed).
            fixed_upsilon_rel_2 (float): Relative curvature threshold (fixed).
        ...
        for n in variable_nr_obs:
            result = self.evaluate_extraction(
                ...
                upsilon_rel_1=fixed_upsilon_rel_1, upsilon_rel_2=fixed_upsilon_rel_2,
    ...
    def sweep_knots(self, fixed_nr_obs, variable_nr_knots, zeta_rel, fixed_upsilon_rel_1, fixed_upsilon_rel_2, upsilon_rel_prune, do_prune):
        ...
        for k in variable_nr_knots:
            result = self.evaluate_extraction(
                fixed_nr_obs, k,
                zeta_rel=zeta_rel,
                upsilon_rel_1=fixed_upsilon_rel_1,
                upsilon_rel_2=fixed_upsilon_rel_2,
                upsilon_rel_prune=upsilon_rel_prune,
                do_prune=do_prune,
            )
            rows.append({
