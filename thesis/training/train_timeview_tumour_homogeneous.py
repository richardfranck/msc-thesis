"""Training run: TimeView baseline, tumour data without unobserved heterogeneity.

Step 1: Searches the encoder architecture and train a MeanOnlyModel with no roughness
penalty, matching Kacprzyk et al., writing models/timeview_tumour_homogeneous
Step 2: Fit NR_MEMBERS bootstrap members at that frozen architecture, and write to
models/bootstrap_timeview_tumour_homogeneous.

    python thesis/training/train_timeview_tumour_homogeneous.py
"""
...
# The baseline carries no roughness penalty at all, matching Kacprzyk et al.
# MeanOnlyModel has no random effects, so lambda_re is zero structurally.
PENALTY = {"lambda_mean": config.LAMBDA_MEAN_UNPENALISED,
           "lambda_re": config.LAMBDA_RE_UNPENALISED}

# Where the two artefacts are written.
MODELS_DIR = THESIS_DIR / "models"
MODEL_PATH = MODELS_DIR / "timeview_tumour_homogeneous"
ENSEMBLE_PATH = MODELS_DIR / "bootstrap_timeview_tumour_homogeneous"
...
if __name__ == "__main__":
    print(f"Training the TimeView baseline on the {REGIME.name} tumour regime")
    run_training()
